...
//     FFMS_Indexer *indexer=FFMS_CreateIndexer(filename,&errinfo);
//     FFMS_Index *index=FFMS_DoIndexing2(indexer,FFMS_IEH_ABORT,&errinfo);
//     int track=FFMS_GetFirstTrackOfType(index,FFMS_TYPE_VIDEO,&errinfo);
//     FFMS_VideoSource *videosource=FFMS_CreateVideoSource(filename,track,index,1,FFMS_SEEK_NORMAL,&errinfo);
//     FFMS_DestroyIndex(index);
//     const FFMS_VideoProperties *videoprops=FFMS_GetVideoProperties(videosource);
//     printf("%d\n",videoprops->CropBottom);
//     int pixfmts[2];
//     pixfmts[0] = FFMS_GetPixFmt("rgb24");
//     pixfmts[1] = -1;
//     FFMS_SetOutputFormatV2(videosource, pixfmts,1280,720,FFMS_RESIZER_BICUBIC, &errinfo);
//     FFMS_Frame *frame=FFMS_GetFrame(videosource,0,&errinfo);
//     printf("%d\n",frame->Linesize[0]);
